...
# column bind (R Uniform Distribution, construct labels)
...
ex1 <- ggplot(val, aes(x, V2)) +
  ...
  xlab("X") +
  ylab("Y")
plot(ex1) # you can see an imaginary ham set contained inside the the horizontal rules
# with spam outside
# maybe
...
get.msg <- function(path)
{
  ...
  # (in our dataset!! `header` is collapsed, no \n in there)
...
{
  control <- list(stopwords = TRUE, # we don't want stop words
                  removePunctuation = TRUE, # we don't want punctuation
                  removeNumbers = TRUE, # we don't want numbers
                  minDocFreq = 2) # if the word only appear once, discard
...
count.word <- function(path, term)
{
  ...
  control <- list(stopwords = TRUE, # we don't want stop words
                  removePunctuation = TRUE, # we don't want punctuation
                  removeNumbers = TRUE) # we don't want numbers
  msg.tdm <- TermDocumentMatrix(msg.corpus, control) # returns a sparse matrix
  word.freq <- rowSums(as.matrix(msg.tdm)) # frecuencies for that particular word
...
classify.email <- function(path, training.df, prior = 0.5, c = 1e-6)
{
  ...
  msg <- get.msg(path)
  msg.tdm <- get.tdm(msg)
  msg.freq <- rowSums(as.matrix(msg.tdm)) # vector of word counts
  # Find intersections of words
  msg.match <- intersect(names(msg.freq), training.df$term) # words that appear in our message vs training
  # Now, we just perform the naive Bayes calculation
  # hold on, who coded this didn't care about readability
  if(length(msg.match) < 1)
  {
    return(prior * c ^ (length(msg.freq))) # c is our close-to-zero value, it's every term!
    # 0.00001^(N) * Pr(SPAM)
  }
  else
  {
    match.probs <- training.df$occurrence[match(msg.match, training.df$term)]
    # likelihood of the terms that appear in the message
    return(prior * prod(match.probs) * c ^ (length(msg.freq) - length(msg.match)))
    # Pr(x1|SPAM)*...*Pr(Xj|SPAM)*0.00001^(N)*Pr(SPAM)
    # prior is anything so SPAM is just an example here above
    #
    # and we return the product of the likelihoods, and multiply "close-to-zero" as many times
    # as the terms we didn't observe (difference in lengths)
  }
}
